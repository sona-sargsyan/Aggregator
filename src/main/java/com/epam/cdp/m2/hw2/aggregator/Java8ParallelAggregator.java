...
public class Java8ParallelAggregator implements Aggregator {
    ...
    public int sum(List<Integer> numbers) {
        return numbers.parallelStream().reduce(0, (c, e) -> (c + e));
    }
    ...
    public List<Pair<String, Long>> getMostFrequentWords(List<String> words, long limit) {
        ...
        pairMap.entrySet().parallelStream()
    ...
    public List<String> getDuplicates(List<String> words, long limit) {
        ...
        duplicates = words.parallelStream().filter(item -> (!items.add(item.toUpperCase()))).map(String::toUpperCase).limit(limit).collect(Collectors.toList());
